...
export default function PartnershipList() {
  ...
  const totalInvested = partnerships.reduce(
    (s, p) => s + parseFloat(p.our_investment || 0),
    0,
  );
  const totalReceivedAll = partnerships.reduce(
    (s, p) => s + parseFloat(p.total_received || 0),
    0,
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <PageHero
        ...
        backTo="/dashboard"
        actions={
          <Button variant="white" onClick={() => navigate("/partnerships/new")}>
            + New Partnership
          </Button>
        }
      >
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
          ...
          <HeroStat
            ...
            value={formatCurrency(totalReceivedAll)}
            accent="teal"
          />
        </div>
      </PageHero>
                  ...
                  <div>
                    ...
                    {p.linked_property_deal_id && (
                      <p className="text-xs text-indigo-600 mt-0.5">
                        🏘 Linked to property deal
                      </p>
                    )}
                  </div>
                  <span
                    className={`shrink-0 px-2 py-0.5 text-xs rounded-full font-medium ${STATUS_COLORS[p.status] || "bg-slate-100 text-slate-700"}`}
                  >
                    {p.status}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                  <div>
                    ...
                    <div className="font-medium text-slate-900">
                      {formatCurrency(p.our_investment || 0)}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs font-medium text-slate-400">
                    ...
                    <div
                      className={`font-medium ${parseFloat(p.total_received) > 0 ? "text-emerald-700" : "text-slate-400"}`}
                    >
