          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="lg:col-span-2 space-y-5">
              ...
              {timelineEvents.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  <h2 className="text-base font-bold text-slate-900 mb-4">Deal Timeline</h2>
                      ...
                      {timelineEvents.map((ev, i) => (
                        <div key={i} className="relative flex items-start gap-3">
                          <div className={`absolute -left-3 top-1 w-4 h-4 rounded-full border-2 border-slate-900 shadow-sm flex items-center justify-center ${ev.done ? `bg-${ev.color}-500` : ev.isPast ? "bg-rose-400" : "bg-amber-400"}`}>
                            {ev.done && (
                              <svg className="w-2 h-2 text-slate-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={4}><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>
                            )}
                          </div>
                          <div>
                            <p className={`text-sm font-semibold ${ev.done ? "text-slate-900" : ev.isPast ? "text-rose-700" : "text-amber-700"}`}>{ev.label}</p>
                            {ev.date && <p className="text-xs text-slate-400">{formatDate(ev.date)}{ev.isPast ? " (overdue)" : ""}</p>}
                            {!ev.done && !ev.isPast && <span className="text-[10px] bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded-full font-semibold border border-amber-200">NEXT</span>}
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-5">
                ...
                {myViewMode && <p className="text-[10px] text-indigo-600 mt-2 text-center">Showing your {selfShare}% share</p>}
              </div>
