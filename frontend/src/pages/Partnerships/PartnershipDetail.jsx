...
const inputCls = "w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-300/40 focus:border-indigo-400 transition-all";
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="space-y-5">
          ...
          {isLinkedToProperty && linkedProperty && (
            <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-900">Linked to property: {linkedProperty.title}</p>
              ...
              <div className="bg-white rounded-2xl border border-violet-200 p-5">
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex justify-between text-xs text-slate-400">
                    <span>Paid <span className="text-violet-700 font-semibold font-mono">{formatCurrency(scale(alreadyPaid))}</span></span>
                    <span>Total <span className="font-semibold font-mono text-slate-600">{formatCurrency(scale(sellerTotal))}</span></span>
                  </div>
                ...
                {sellerTxns.length > 0 && (
                  <div className="border-t border-slate-200 pt-3 space-y-2">
                    ...
                    {sellerTxns.map((t) => (
                      <div key={t.id} className="flex justify-between items-center py-1.5 border-b border-slate-100 last:border-0">
                        <div>
                          <span className="text-xs font-medium text-violet-700 bg-violet-500/10 px-2 py-0.5 rounded-full border border-violet-200">
                            {TXN_TYPE_LABELS[t.txn_type] || t.txn_type}
                          </span>
                          {t.description && <p className="text-[11px] text-slate-400 mt-0.5">{t.description}</p>}
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-bold font-mono text-rose-700">−{formatCurrency(scale(parseFloat(t.amount || 0)))}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="lg:col-span-2 space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-base font-bold text-slate-900">Partner Net Position</h2>
                  {isActive && (
                    <button onClick={() => setShowAddMemberModal(true)} className="px-3 py-1.5 bg-white text-slate-600 border border-slate-200 rounded-xl text-sm hover:bg-slate-100">+ Add Partner</button>
                  )}
                </div>
                      ...
                      return (
                        <div key={i} className={`rounded-xl border p-4 ${m.member?.is_self ? "bg-indigo-50 border-indigo-200" : "bg-slate-50 border-slate-200"}`}>
                          ...
                          <div className="flex items-center gap-1 text-xs flex-wrap mb-2">
                            <span className="font-mono text-slate-600 bg-slate-100 rounded px-1.5 py-0.5">{formatCurrency(advance)}</span>
                            ...
                            {withdrawals > 0 && <>
                              <span className="text-slate-500">−</span>
                              <span className="font-mono text-amber-700 bg-amber-50 rounded px-1.5 py-0.5 border border-amber-200">{formatCurrency(withdrawals)}</span>
                              <span className="text-slate-500">out</span>
                            </>}
                            <span className="text-slate-500">=</span>
                            <span className={`font-mono font-bold rounded px-1.5 py-0.5 ${currentStake > 0 ? "text-emerald-700 bg-emerald-50 border border-emerald-200" : "text-rose-700 bg-rose-50 border border-rose-200"}`}>{formatCurrency(currentStake)}</span>
                          ...
                          {isActive && (
                            <div className="flex gap-2 mt-2 pt-2 border-t border-slate-200">
                              ...
                              <button onClick={() => { if (window.confirm(`Remove ${name}?`)) deleteMemberMutation.mutate(memberId); }} className="text-xs text-rose-700 hover:underline">Delete</button>
                            </div>
                          )}
                        </div>
                      );
              ...
              {timelineEvents.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  <h2 className="text-base font-bold text-slate-900 mb-5">Deal Timeline</h2>
                  <div className="space-y-0">
                    {timelineEvents.map((ev, i) => {
                      const isLast = i === timelineEvents.length - 1;
                      const doneCls = {
                        indigo: "bg-indigo-50 border-indigo-300 text-indigo-600",
                        purple: "bg-violet-50 border-violet-300 text-violet-600",
                        emerald: "bg-emerald-50 border-emerald-300 text-emerald-600",
                        teal:   "bg-teal-50 border-teal-300 text-teal-600",
                      }[ev.color] || "bg-slate-50 border-slate-300 text-slate-500";
                      const circleCls = ev.done
                        ? doneCls
                        : ev.isPast
                      ...
                      return (
                        <div key={i} className="flex gap-4">
                          ...
                          <div className="flex flex-col items-center w-8 shrink-0">
                            <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center shrink-0 ${circleCls}`}>
                              {ev.done ? (
                            ...
                            {!isLast && <div className="w-px flex-1 min-h-[20px] bg-slate-200 my-1" />}
                          </div>
                          {/* content */}
                          <div className={`min-w-0 ${isLast ? "pb-0" : "pb-5"}`}>
                            <p className={`text-sm font-semibold leading-snug ${ev.done ? "text-slate-800" : ev.isPast ? "text-rose-700" : "text-amber-700"}`}>
                              {ev.label}
                            </p>
                            {ev.date && (
                              <p className="text-xs text-slate-400 mt-0.5">
                                {formatDate(ev.date)}{ev.isPast ? " · overdue" : ""}
                              </p>
                            )}
                            {!ev.done && !ev.isPast && (
                              <span className="mt-1.5 inline-flex text-[10px] font-bold tracking-wider bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full border border-amber-200">
                                UPCOMING
                              </span>
                            )}
                          </div>
                  ...
                  {assigningBuyerTo && (
                    <div className="mb-4 p-4 bg-amber-50/50 rounded-xl border border-amber-200/60 space-y-3">
                      ...
                      <div className="flex gap-2 mb-2">
                        <button onClick={() => setAssignBuyerMode("existing")} className={`px-3 py-1 rounded-lg text-xs font-medium ${assignBuyerMode === "existing" ? "bg-amber-500/30 text-amber-300" : "bg-white text-slate-400 border border-slate-200"}`}>Pick Existing Contact</button>
                        <button onClick={() => setAssignBuyerMode("new")} className={`px-3 py-1 rounded-lg text-xs font-medium ${assignBuyerMode === "new" ? "bg-amber-500/30 text-amber-300" : "bg-white text-slate-400 border border-slate-200"}`}>Create New Contact</button>
                      </div>
                        ...
                        {plotBuyers.map((b) => {
                          ...
                          return (
                            <div key={b.id} className="border border-slate-200 rounded-xl overflow-hidden bg-slate-50">
                              <div className="p-3 cursor-pointer hover:bg-white transition-colors" onClick={() => setExpandedPlotId(isExpanded ? null : plotKey)}>
                                <div className="flex justify-between items-start">
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <p className="text-sm font-semibold text-slate-900">{b.buyer_name || <span className="text-slate-400 italic">No buyer assigned</span>}</p>
                                    </div>
                                    <p className="text-xs text-slate-500 ml-4">{b.area_sqft ? `${b.area_sqft} sq ft` : ""}{b.rate_per_sqft ? ` @ ₹${b.rate_per_sqft}/sqft` : ""}</p>
                                  </div>
                                  <div className="text-right ml-3 shrink-0">
                                    <p className="text-sm font-bold font-mono text-slate-700">{formatCurrency(scale(totalValue))}</p>
                                    <p className="text-xs text-emerald-600 font-mono">Paid: {formatCurrency(scale(totalPaid))}</p>
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${b.status === "registry_done" ? "bg-emerald-50 text-emerald-700" : b.status === "advance_received" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{(b.status || "negotiating").replace(/_/g, " ")}</span>
                                  </div>
                                </div>
                                {totalValue > 0 && (
                                  <div className="mt-2 ml-4">
                                    ...
                                    <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-emerald-400 to-emerald-500 rounded-full transition-all duration-500" style={{ width: `${paidPct}%` }} /></div>
                                  </div>
                                )}
                    ...
                    sitePlots.length > 0 ? (
                      <div className="space-y-3">
                        {sitePlots.map((sp) => {
                          ...
                          return (
                            <div key={sp.id} className="border border-slate-200 rounded-xl overflow-hidden bg-slate-50">
                              <div className="p-3 cursor-pointer hover:bg-white transition-colors" onClick={() => setExpandedPlotId(isExpanded ? null : plotKey)}>
                                <div className="flex justify-between items-start">
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <p className="text-sm font-semibold text-slate-900">{sp.plot_number || `Plot #${sp.id}`}</p>
                                  ...
                                  <div className="text-right ml-3 shrink-0">
                                    <p className="text-sm font-bold font-mono text-slate-700">{formatCurrency(scale(totalValue))}</p>
                                    <p className="text-xs text-emerald-600 font-mono">Paid: {formatCurrency(scale(totalPaid))}</p>
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${sp.status === "sold" ? "bg-emerald-50 text-emerald-700" : sp.status === "advance_received" ? "bg-amber-50 text-amber-700" : sp.status === "available" ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-500"}`}>{(sp.status || "available").replace(/_/g, " ")}</span>
                                  </div>
                                </div>
                                {totalValue > 0 && (
                                  <div className="mt-2 ml-4">
                                    <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-teal-400 to-teal-500 rounded-full transition-all duration-500" style={{ width: `${paidPct}%` }} /></div>
                                  </div>
                                )}
                                <div className="flex gap-2 mt-2 ml-4 flex-wrap">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  {isActive && (
                    <button
                      ...
                      className="px-3 py-1.5 bg-white text-slate-600 border border-slate-200 rounded-xl text-sm hover:bg-slate-100"
                    >
                  ...
                  <div className="mb-5 p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                    <div className="grid grid-cols-3 gap-3">
                      <div className="col-span-1">
                        ...
                        <select value={txnForm.txn_type} onChange={(e) => setTxnForm(p => ({ ...p, txn_type: e.target.value, member_id: "", received_by_member_id: "", plot_buyer_id: "", site_plot_id: "", account_id: "", broker_name: "", from_partnership_pot: false }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-sm text-slate-700 placeholder-slate-400">
                      ...
                      <div>
                        ...
                        <input type="date" value={txnForm.txn_date} onChange={(e) => setTxnForm(p => ({ ...p, txn_date: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-sm text-slate-700 placeholder-slate-400" />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      {txnForm.txn_type === "partner_transfer" && (
                        <>
                          <div>
                            ...
                            <select value={txnForm.member_id} onChange={(e) => setTxnForm(p => ({ ...p, member_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-sm text-slate-700 placeholder-slate-400">
                          ...
                          <div>
                            ...
                            <select value={txnForm.received_by_member_id} onChange={(e) => setTxnForm(p => ({ ...p, received_by_member_id: e.target.value, account_id: "" }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-sm text-slate-700 placeholder-slate-400">
                              <option value="">— Select —</option>
                          ...
                          {(() => { const toMember = members.find(m => String(m.member?.id) === String(txnForm.received_by_member_id)); return toMember?.member?.is_self && accounts.length > 0; })() && (
                            <div>
                              ...
                              <select value={txnForm.account_id} onChange={(e) => setTxnForm(p => ({ ...p, account_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-sm text-slate-700 placeholder-slate-400">
                      ...
                      {INFLOW_TYPES.includes(txnForm.txn_type) && (
                        <div>
                          ...
                          <select value={txnForm.received_by_member_id} onChange={(e) => setTxnForm(p => ({ ...p, received_by_member_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-sm text-slate-700 placeholder-slate-400">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {plotBuyers.length > 0 && (
                          <div>
                            ...
                            <select value={txnForm.plot_buyer_id} onChange={(e) => setTxnForm(p => ({ ...p, plot_buyer_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-sm text-slate-700 placeholder-slate-400">
                              <option value="">— None —</option>
                        ...
                        {sitePlots.length > 0 && (
                          <div>
                            ...
                            <select value={txnForm.site_plot_id} onChange={(e) => setTxnForm(p => ({ ...p, site_plot_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-sm text-slate-700 placeholder-slate-400">
                              <option value="">— None —</option>
                    ...
                    <div>
                      ...
                      <input type="text" value={txnForm.description} onChange={(e) => setTxnForm(p => ({ ...p, description: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-sm text-slate-700 placeholder-slate-400" placeholder="Optional" />
                    </div>
                                    ...
                                    {editingTxnId === txn.id && editTxnForm ? (
                                      <div className="space-y-2">
                                        <div className="grid grid-cols-3 gap-2">
                                          <div>
                                            <label className="block text-xs text-slate-500 mb-0.5">Type</label>
                                            <select value={editTxnForm.txn_type} onChange={(e) => setEditTxnForm(p => ({ ...p, txn_type: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 text-sm text-slate-700">
                                              <optgroup label="Outflows">
                                                <option value="advance_to_seller">Advance to Seller</option>
                                                <option value="remaining_to_seller">Remaining to Seller</option>
                                                <option value="broker_commission">Broker Commission</option>
                                          ...
                                          <div>
                                            <label className="block text-xs text-slate-500 mb-0.5">Amount</label>
                                            <input type="number" value={editTxnForm.amount} onChange={(e) => setEditTxnForm(p => ({ ...p, amount: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 text-sm text-slate-700" />
                                          </div>
                                          <div>
                                            <label className="block text-xs text-slate-500 mb-0.5">Date</label>
                                            <input type="date" value={editTxnForm.txn_date} onChange={(e) => setEditTxnForm(p => ({ ...p, txn_date: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 text-sm text-slate-700" />
                                          </div>
                                        </div>
                                        <div className="grid grid-cols-3 gap-2">
                                          {OUTFLOW_TYPES.includes(editTxnForm.txn_type) && (
                                            <div>
                                              <label className="block text-xs text-slate-500 mb-0.5">Paid by</label>
                                              <select value={editTxnForm.member_id} onChange={(e) => setEditTxnForm(p => ({ ...p, member_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 text-sm text-slate-700">
                                                <option value="">— Select —</option>
                                                {members.map(m => <option key={m.member?.id} value={String(m.member?.id)}>{m.member?.is_self ? "Self" : m.contact?.name || "Partner"}</option>)}
                                              </select>
                                            </div>
                                          ...
                                          {editTxnForm.txn_type === "partner_transfer" && (
                                            <>
                                              ...
                                              <div>
                                                <label className="block text-xs text-slate-500 mb-0.5">To</label>
                                                <select value={editTxnForm.received_by_member_id} onChange={(e) => setEditTxnForm(p => ({ ...p, received_by_member_id: e.target.value, account_id: "" }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 text-sm text-slate-700">
                                                  <option value="">— Select —</option>
                                              ...
                                              {(() => { const toMember = members.find(m => String(m.member?.id) === String(editTxnForm.received_by_member_id)); return toMember?.member?.is_self && accounts.length > 0; })() && (
                                                <div>
                                                  <label className="block text-xs text-slate-500 mb-0.5">Into Account</label>
                                                  <select value={editTxnForm.account_id} onChange={(e) => setEditTxnForm(p => ({ ...p, account_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 text-sm text-slate-700">
                                                    <option value="">— None —</option>
                                                    {accounts.map(a => <option key={a.id} value={String(a.id)}>{a.name}</option>)}
                                                  </select>
                                                </div>
                                          ...
                                          {INFLOW_TYPES.includes(editTxnForm.txn_type) && (
                                            <div>
                                              <label className="block text-xs text-slate-500 mb-0.5">Received by</label>
                                              <select value={editTxnForm.received_by_member_id} onChange={(e) => setEditTxnForm(p => ({ ...p, received_by_member_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 text-sm text-slate-700">
                                                <option value="">Self (Me)</option>
                                                {members.filter(m => !m.member?.is_self).map(m => <option key={m.member?.id} value={String(m.member?.id)}>{m.contact?.name || "Partner"}</option>)}
                                        ...
                                        {["buyer_advance", "buyer_payment"].includes(editTxnForm.txn_type) && (plotBuyers.length > 0 || sitePlots.length > 0) && (
                                          <div className="grid grid-cols-2 gap-2">
                                            {plotBuyers.length > 0 && <div><label className="block text-xs text-slate-500 mb-0.5">Plot Buyer</label><select value={editTxnForm.plot_buyer_id} onChange={(e) => setEditTxnForm(p => ({ ...p, plot_buyer_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 text-sm text-slate-700"><option value="">— None —</option>{plotBuyers.map(b => <option key={b.id} value={b.id}>{b.buyer_name || `Buyer #${b.id}`}</option>)}</select></div>}
                                            {sitePlots.length > 0 && <div><label className="block text-xs text-slate-500 mb-0.5">Site Plot</label><select value={editTxnForm.site_plot_id} onChange={(e) => setEditTxnForm(p => ({ ...p, site_plot_id: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 text-sm text-slate-700"><option value="">— None —</option>{sitePlots.map(sp => <option key={sp.id} value={sp.id}>{sp.plot_number || sp.buyer_name || `Plot #${sp.id}`}</option>)}</select></div>}
                                          </div>
                                        )}
                                        {editTxnForm.txn_type === "broker_commission" && (
                                          <div className="grid grid-cols-2 gap-2">
                                            <InputField label="Broker Name"><input type="text" value={editTxnForm.broker_name} onChange={(e) => setEditTxnForm(p => ({ ...p, broker_name: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-2 py-1 text-sm text-slate-700" placeholder="Broker name" /></InputField>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-5">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex justify-between py-1.5 border-b border-slate-200">
                    <span className="text-sm text-emerald-700 font-medium">Buyer Payments</span>
                    <span className="text-sm font-mono text-emerald-700">+{formatCurrency(scale(parseFloat(summary.buyer_inflow || 0)))}</span>
                  </div>
                  <div className="flex justify-between py-1.5 border-b border-slate-200">
                    <span className="text-sm text-teal-400 font-medium">Profit Received</span>
                    <span className="text-sm font-mono text-emerald-700">+{formatCurrency(scale(parseFloat(summary.profit_received || 0)))}</span>
                  </div>
                  <div className="flex justify-between py-2">
                    <span className="text-sm font-bold text-slate-600">Total Inflow</span>
                    <span className="text-sm font-bold font-mono text-emerald-700">+{formatCurrency(scale(totalInflow))}</span>
                  </div>
                  <div className={`flex justify-between py-2 border-t-2 ${netPnl >= 0 ? "border-emerald-200" : "border-rose-200"}`}>
                    <span className="text-sm font-bold text-slate-900">Net P&L</span>
                    <span className={`text-base font-bold font-mono ${netPnl >= 0 ? "text-emerald-700" : "text-rose-700"}`}>{formatCurrency(scale(netPnl))}</span>
                  </div>
                </div>
              ...
              {/* Details */}
              <div className="bg-white rounded-2xl border border-slate-200 p-5">
                <h2 className="text-base font-bold text-slate-900 mb-3">Details</h2>
                <div className="space-y-2 text-sm">
                  {partnership.start_date && <div className="flex justify-between"><span className="text-slate-500">Start Date</span><span className="font-medium text-slate-700">{formatDate(partnership.start_date)}</span></div>}
                  {partnership.expected_end_date && <div className="flex justify-between"><span className="text-slate-500">Expected End</span><span className={`font-medium ${new Date(partnership.expected_end_date) < new Date() && isActive ? "text-amber-700 font-bold" : "text-slate-700"}`}>{formatDate(partnership.expected_end_date)}</span></div>}
                  {isSettled && partnership.actual_end_date && <div className="flex justify-between"><span className="text-slate-500">Settled</span><span className="font-medium text-emerald-700">{formatDate(partnership.actual_end_date)}</span></div>}
              ...
              {partnership.notes && (
                <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  <h2 className="text-base font-bold text-slate-900 mb-2">Notes</h2>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-50 rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md">
            ...
            <div className="p-5 border-t border-slate-200 flex gap-3 justify-end">
              <button onClick={() => setEditingPlot(null)} className="px-4 py-2 bg-white text-slate-600 rounded-xl text-sm font-medium hover:bg-slate-100">Cancel</button>
              <button
                onClick={() => {
                  const payload = {};
      ...
      {viewingPlot && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-slate-50 rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${vp.status === "sold" || vp.status === "registry_done" ? "bg-emerald-50 text-emerald-700" : vp.status === "advance_received" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-500"}`}>
              ...
              <div className="p-5 border-t border-slate-200 flex justify-end">
                <button onClick={() => setViewingPlot(null)} className="px-4 py-2 bg-white text-slate-600 rounded-xl text-sm font-medium hover:bg-slate-100">Close</button>
              </div>
            </div>
          </div>
        );
      ...
      {closeDealPlot && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-50 rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md">
            ...
            <div className="p-5 border-t border-slate-200 flex gap-3 justify-end">
              <button onClick={() => { setCloseDealPlot(null); setCloseDealForm({ area_sqft: "", price_per_sqft: "", registry_date: "", notes: "" }); }} className="px-4 py-2 bg-white text-slate-600 rounded-xl text-sm font-medium hover:bg-slate-100">Cancel</button>
              <button
                onClick={() => {
