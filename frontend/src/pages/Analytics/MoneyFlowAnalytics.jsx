...
export default function MoneyFlowAnalytics() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () =>
      (await api.get("/api/analytics/money-flow", { params })).data,
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Track every rupee coming in and going out across all accounts
              </p>
            </div>
          </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-xl border p-5">
                  ...
                  <p
                    className={`text-2xl font-bold mt-1 ${data.net_flow >= 0 ? "text-green-600" : "text-red-600"}`}
                  >
                    {data.net_flow >= 0 ? "+" : ""}
                    {fc(data.net_flow)}
                  </p>
              ...
              {data.monthly?.length > 0 && (
                <div className="bg-white rounded-xl border p-5">
                  ...
                  <p className="text-xs text-gray-400 mb-4">
                    Credits vs Debits across all accounts
                  </p>
                  <div className="h-72">
                    ...
                    <>
                      <div className="h-48 mb-4">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={data.inflow_by_source.map((s) => ({
                                name: fmt(s.source),
                                value: s.amount,
                              }))}
                              cx="50%"
                    ...
                    <>
                      <div className="h-48 mb-4">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={data.outflow_by_source.map((s) => ({
                                name: fmt(s.source),
                                value: s.amount,
                              }))}
                              cx="50%"
                              ...
                              labelLine={false}
                            >
                              {data.outflow_by_source.map((_, i) => (
                                <Cell
                                  key={i}
                                  fill={COLORS[i % COLORS.length]}
                                />
              ...
              {data.by_account?.length > 0 && (
                <div className="bg-white rounded-xl border p-5">
                  ...
                  <p className="text-xs text-gray-400 mb-4">
                    Money movement through each account
                  </p>
                    ...
                    <table className="w-full text-sm min-w-[500px]">
                      <thead>
                        <tr className="border-b border-gray-100 text-left">
                          ...
                          <th className="pb-3 text-right text-xs font-semibold text-gray-400 uppercase">
                            Credits
                          </th>
                          <th className="pb-3 text-right text-xs font-semibold text-gray-400 uppercase">
                            Debits
                      ...
                      <tbody>
                        {data.by_account.map((row) => (
                          ...
                          >
                            ...
                            <td
                              className={`py-2.5 text-right font-semibold ${row.net >= 0 ? "text-green-700" : "text-red-700"}`}
                            >
                              {row.net >= 0 ? "+" : ""}
                              {fc(row.net)}
                            </td>
              ...
              {data.expenses?.by_category?.length > 0 && (
                <div className="bg-white rounded-xl border p-5">
                  ...
                  <p className="text-xs text-gray-400 mb-4">
                    Spending by category &mdash; Total:{" "}
                    {fc(data.expenses.total)}
                  </p>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="h-56">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                            data={data.expenses.by_category.map((c) => ({
                              name: c.category,
                              value: c.amount,
                            }))}
                            cx="50%"
                            ...
                            labelLine={false}
                          >
                            {data.expenses.by_category.map((_, i) => (
                      ...
                      {data.expenses.by_category.map((c, i) => {
                        const pct =
                          data.expenses.total > 0
                            ? (c.amount / data.expenses.total) * 100
                            : 0;
                        return (
                          <div key={c.category}>
              ...
              {data.recent_transactions?.length > 0 && (
                <div className="bg-white rounded-xl border p-5">
                  ...
                  <p className="text-xs text-gray-400 mb-4">
                    Last 50 account movements
                  </p>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm min-w-[700px]">
                      <thead>
                        <tr className="border-b border-gray-100 text-left">
                          <th className="pb-2 text-xs font-semibold text-gray-400 uppercase">
                            Date
                          </th>
                          <th className="pb-2 text-xs font-semibold text-gray-400 uppercase">
                            Account
                          ...
                          <th className="pb-2 text-right text-xs font-semibold text-gray-400 uppercase">
                            Amount
                          </th>
                        </tr>
                      </thead>
                        ...
                        {data.recent_transactions.map((t, i) => (
                          ...
                          >
                            ...
                            <td className="py-2 text-gray-500 capitalize text-xs">
                              {(t.payment_mode || "").replace("_", " ")}
                            </td>
                            <td
                              className={`py-2 text-right font-semibold ${t.type === "credit" ? "text-green-600" : "text-red-600"}`}
                            >
                              {t.type === "credit" ? "+" : "-"}
                              {fc(t.amount)}
                            </td>
