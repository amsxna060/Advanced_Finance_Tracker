          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-5 sm:p-6">
              ...
              {loan.loan_type === "emi" && emiInterestSummary && (
                <EmiSummarySection emiInterestSummary={emiInterestSummary} loan={loan} />
              )}
              {loan.notes && (
            ...
            {loan.loan_type === "emi" && emiSchedule.length > 0 && (
              <EmiScheduleSection emiSchedule={emiSchedule} loan={loan} />
            )}
...
function EmiSummarySection({ emiInterestSummary: s, loan }) {
  const [showAmort, setShowAmort] = useState(false);

  return (
    <div className="mt-4 pt-4 border-t border-slate-100">
      <div className="text-sm font-medium text-slate-700 mb-2">EMI Interest Summary</div>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm mb-3">
        <div className="bg-indigo-50 rounded-xl p-3">
          <div className="text-xs text-slate-500">Total Repayment</div>
          <div className="font-semibold text-slate-900">{formatCurrency(s.total_repayment)}</div>
        </div>
        <div className="bg-amber-50 rounded-xl p-3">
          <div className="text-xs text-slate-500">Total Interest</div>
          <div className="font-semibold text-slate-900">{formatCurrency(s.total_interest_embedded)}</div>
        </div>
        <div className="bg-violet-50 rounded-xl p-3">
          <div className="text-xs text-slate-500">Effective Rate (reducing)</div>
        ...
        {loan.penalty_per_day && (
          <div className="bg-rose-50 rounded-xl p-3">
            ...
            <div className="font-semibold text-rose-700">₹{loan.penalty_per_day}/day</div>
          </div>
        )}
      </div>
      {s.amortization?.length > 0 && (
        <div>
          <button
            onClick={() => setShowAmort((v) => !v)}
            className="text-xs text-indigo-600 hover:text-indigo-800 font-medium"
          >
            {showAmort ? "Hide" : "Show"} Amortization (principal vs interest per EMI)
          </button>
          {showAmort && (
            <div className="mt-3 overflow-x-auto">
              <table className="min-w-full text-xs divide-y divide-slate-100">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="px-3 py-2 text-left font-medium text-slate-500 uppercase">#</th>
                    <th className="px-3 py-2 text-right font-medium text-slate-500 uppercase">Interest</th>
                    <th className="px-3 py-2 text-right font-medium text-slate-500 uppercase">Principal</th>
                    <th className="px-3 py-2 text-right font-medium text-slate-500 uppercase">Balance</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {s.amortization.map((row) => (
                    <tr key={row.emi_number} className="hover:bg-slate-50/50">
                      <td className="px-3 py-1.5 text-slate-600">{row.emi_number}</td>
                      <td className="px-3 py-1.5 text-right text-amber-600">{formatCurrency(row.interest_component)}</td>
                      <td className="px-3 py-1.5 text-right text-indigo-600">{formatCurrency(row.principal_component)}</td>
                      <td className="px-3 py-1.5 text-right text-slate-700 font-medium">{formatCurrency(row.outstanding_after)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              ...
              {hasPenalty && (
                <th className="px-4 py-3 text-left text-xs font-medium text-rose-500 uppercase">Penalty</th>
              )}
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {emiSchedule.map((entry) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-sm">
                  {entry.status === "paid" ? (
                    <span className="px-2 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-medium">✓ Paid</span>
                  ) : entry.status === "partial" ? (
                    <span className="px-2 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-medium">⚡ Partial</span>
                  ) : entry.status === "future" ? (
                    <span className="px-2 py-1 bg-slate-100 text-slate-500 rounded-full text-xs font-medium">Future</span>
                  ) : (
                    <span className="px-2 py-1 bg-rose-100 text-rose-700 rounded-full text-xs font-medium">✗ Unpaid</span>
                  )}
                </td>
              </tr>
            ))}
