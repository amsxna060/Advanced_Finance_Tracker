...
function SitePlotsSection({ propertyId, plots, accounts, isSettled }) {
  ...
  const set = (k, v) => {
    setForm((prev) => {
      const next = { ...prev, [k]: v };
      if (
        (k === "area_sqft" || k === "sold_price_per_sqft") &&
        next.area_sqft &&
        next.sold_price_per_sqft
      ) {
        next.calculated_price = String(
          (parseFloat(next.area_sqft) || 0) *
            (parseFloat(next.sold_price_per_sqft) || 0),
        );
      }
      return next;
    });
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload) => {
      if (editingId) {
        return api.put(
          `/api/properties/${propertyId}/plots/${editingId}`,
          payload,
        );
      }
    ...
    onError: (err) =>
      alert(err?.response?.data?.detail || "Failed to save plot"),
  });

  const deleteMutation = useMutation({
    mutationFn: async (plotId) =>
      api.delete(`/api/properties/${propertyId}/plots/${plotId}`),
  ...
  const openEdit = (p) => {
    ...
    setForm({
      ...
      sold_price_per_sqft: p.sold_price_per_sqft
        ? String(p.sold_price_per_sqft)
        : "",
  ...
  const totalRevenue = plots.reduce(
    (s, p) => s + parseFloat(p.calculated_price || 0),
    0,
  );
  const totalAreaSold = plots.reduce(
    (s, p) => s + parseFloat(p.area_sqft || 0),
    0,
  );
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div>
              ...
              <input
                type="number"
                step="0.001"
                value={form.sold_price_per_sqft}
                onChange={(e) => set("sold_price_per_sqft", e.target.value)}
                className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-400 transition-all"
                min="0"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => set("calculated_price", e.target.value)}
                className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-400 transition-all"
                min="0"
              />
            </div>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                ...
                <tr className="border-t border-slate-300 font-semibold">
                  ...
                  <td className="text-right py-2">
                    {totalAreaSold.toLocaleString()} sqft
                  </td>
                  <td></td>
...
export default function PropertyDetail() {
  ...
  const calculatePlotSettlementSummary = () => {
    ...
    const partnerPartnershipExpTotal = partnershipExpenses.reduce(
      (s, pe) => s + pe.amount,
      0,
    );
    const propLevelExpTotal =
      parseFloat(property.other_expenses || 0) - partnerPartnershipExpTotal;
  ...
  const otherExpenseTxns = transactions.filter(
    (t) => t.txn_type === "other_expense",
  );
  ...
  const totalOtherExpenses = allOtherExpenses.reduce(
        ...
        <div className="mt-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
          {isSite ? (
            <>
              <HeroStat
                label="My Investment"
                value={formatCurrency(property.my_investment || 0)}
                accent="indigo"
              />
              <HeroStat
                label="Total Profit Received"
                value={formatCurrency(property.total_profit_received || 0)}
                accent="emerald"
              />
              <HeroStat
                label="My Share"
                value={`${property.my_share_percentage || 0}%`}
                accent="violet"
              />
              <HeroStat
                label="Status"
                value={property.status?.replace(/_/g, " ")}
                accent={isSettled ? "teal" : "amber"}
              />
            </>
          ) : (
            <>
              <HeroStat
                ...
                value={formatCurrency(property.total_seller_value || 0)}
                accent="indigo"
              />
              <HeroStat
                label="Advance Paid"
                value={formatCurrency(property.advance_paid || 0)}
                accent="amber"
              />
              <HeroStat
                label="Net Profit"
                ...
                accent={
                  parseFloat(property.net_profit || 0) >= 0 ? "teal" : "rose"
                }
              />
              <HeroStat
                label="Broker Commission"
                value={formatCurrency(property.broker_commission || 0)}
                accent="violet"
              />
            </>
          )}
                  ...
                  <>
                    ...
                    <div className="flex justify-center mb-4">
                      <PlotDiagram
                        ...
                        bottom={
                          property.side_south_ft || property.side_bottom_ft
                        }
                ...
                {allOtherExpenses.length > 0 ? (
                  <>
                    <table className="min-w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-200">
                          ...
                          <th className="text-left py-2 text-slate-500 font-medium pl-2">
                            Description
                          </th>
                          <th className="text-left py-2 text-slate-500 font-medium pl-2">
                            Paid by
                      ...
                      <tbody>
                        {allOtherExpenses.map((t) => (
                          ...
                          >
                            ...
                            <td className="py-2 text-slate-600 pl-2">
                              {t.description || "—"}
                            </td>
                            <td className="py-2 pl-2">
                              <span
                                className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${t.source === "partnership" ? "bg-purple-50 text-purple-700" : "bg-slate-100 text-slate-600"}`}
                              >
                                {t.payer_name || "Self"}
                        ...
                        <tr className="border-t border-slate-300 font-semibold">
                          ...
                          <td className="text-right py-2 text-orange-700">
                    ...
                    <p className="text-xs text-orange-600 mt-2">
                      ℹ️ Each person's expenses are returned to them at
                      settlement, before profit sharing.
                    </p>
                  </>
                ) : (
                  <p className="text-sm text-slate-400 italic">
                  ...
                  <button
                    onClick={() => {
                      setSettleForm((p) => ({
                        ...p,
                        other_expenses:
                          totalOtherExpenses > 0 && p.other_expenses === "0"
                        ...
                        {members.length > 0 && liveNet !== null && (
                          <>
                            ...
                            {members.map((m, i) => {
                              ...
                              const propExpBack = m.member.is_self
                                ? (isNaN(liveOther) ? 0 : liveOther) -
                                  partnershipExpenses
                                    .filter((pe) => pe.is_self)
                                    .reduce((s, pe) => s + pe.amount, 0)
                                : 0;
