  ...
  return (
    <div
      ...
      className={cn(
        ...
        hover &&
          "hover:shadow-md hover:border-slate-300/60 transition-all cursor-pointer",
...
export function StatCard({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-slate-200/60 shadow-sm border-l-4 px-5 py-4",
        accents[accent],
        className,
      )}
    >
      ...
      <p className="text-xl sm:text-2xl font-extrabold text-slate-800 mt-1 tracking-tight">
        {value}
      </p>
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            {subtitle && (
              <p className="text-indigo-300/70 text-sm mt-1">{subtitle}</p>
            )}
          </div>
          {actions && (
...
export function HeroStat({ label, value, sub, accent }) {
  ...
  return (
    <div
      className={cn(
        "bg-white/[0.07] backdrop-blur-xl border border-white/[0.12] rounded-2xl px-5 py-4",
...
export function PageBody({ children, className }) {
  return (
    <div
      className={cn(
        "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-5 pb-16 relative z-10",
        className,
      )}
    >
      {children}
...
const buttonVariants = {
  primary:
    "bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm shadow-indigo-500/20 active:scale-[0.98]",
  white:
    "bg-white text-slate-800 hover:bg-slate-50 shadow-sm active:scale-[0.98]",
  secondary:
  ...
  success:
    "bg-emerald-600 text-white hover:bg-emerald-700 shadow-sm shadow-emerald-500/20 active:scale-[0.98]",
};
...
export const Select = forwardRef(
  ({ label, error, className, children, labelClassName, ...props }, ref) => (
    <div className="space-y-1.5">
      {label && (
        <label
          className={
...
export function StatusBadge({ status, className }) {
  ...
  return (
    <Badge variant={m.variant} dot className={className}>
      {m.label}
    </Badge>
  );
}
...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        className={cn(
          "relative bg-white rounded-2xl shadow-2xl w-full animate-slideDown overflow-hidden",
          sizes[size],
          className,
        )}
      >
...
export function Tabs({ tabs, active, onChange, className }) {
  return (
    <div
      className={cn("flex gap-1 p-1 bg-slate-100 rounded-xl w-fit", className)}
    >
      {tabs.map((t) => (
        <button
          key={t.key}
...
export function Th({ children, className }) {
  return (
    <th
      className={cn(
        "px-5 py-3 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider bg-slate-50/60",
        className,
      )}
    >
      {children}
...
export function Td({ children, className }) {
  return (
    <td className={cn("px-5 py-3.5 text-sm text-slate-700", className)}>
      {children}
    </td>
  );
}
...
export function InfoRow({ label, value, className }) {
  return (
    <div
      className={cn(
        "flex justify-between items-center py-2.5 border-b border-slate-50 last:border-0",
        className,
      )}
    >
...
export function SectionHeader({ title, count, children, className }) {
  return (
    <div
      className={cn("flex items-center justify-between mb-4 mt-8", className)}
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-24 bg-white rounded-2xl border border-slate-200/60"
            />
          ))}
        </div>
